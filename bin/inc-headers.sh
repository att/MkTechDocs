...
##
# A utility script that increments the headers in a given
...
if [[ "$1" == "" ]] ; then
	echo "usage: inc_headers.sh <markdown file> [n]"
	echo "    Increments headers in a given markdown file by n levels."
...
pandoc -f markdown -t markdown --wrap=none --filter flt-increment-header-${N}.py $FILE > $$.md
cp $FILE $FILE.$$
mv $$.md $FILE
